...
public abstract class InfoDictionary {
	...
	// The filename (in Single File Mode) or The file path of the directory (in
	// Multiple File Mode)
	private String name;
	/*
	 * (optional) If it is set to "1", the client MUST publish its presence to get
	 * other peers ONLY via the trackers explicitly described in the metainfo file.
	 * If this field is set to "0" or is not present, the client may obtain peer
	 * from other means, e.g. peer exchange.
	 */
	...
	/*
	 * urlencoded 20-byte SHA1 hash of the value of the info key from the Metainfo
	 * file. Note that the value will be a bencoded dictionary.
	 */
	...
	public String toString() {
		...
		if (!this.byteSHA1.isEmpty() || !this.hexStringSHA1.isEmpty() || !this.urlEncodedSHA1.isEmpty()) {
			buffer.append("\nnumber of pieces: ");
